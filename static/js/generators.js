...
Blockly.JavaScript['normalize_data'] = function(block) {
  ...
  if (method === 'standard') {
    ...
    code += `  const mean = dataTensor.mean(0);\n`;
    code += `  const std = dataTensor.sub(mean).square().mean(0).sqrt();\n`;
    code += `  const normalized = dataTensor.sub(mean).div(std);\n`;
...
Blockly.JavaScript['create_linear_model'] = function(block) {
  ...
  if (modelType === 'linear_regression') {
    ...
    code += `  model.add(tf.layers.dense({units: 1, inputShape: [null], activation: 'linear'}));\n`;
    code += `  model.compile({\n`;
    code += `    optimizer: tf.train.adam(0.01),\n`;
    code += `    loss: 'meanSquaredError',\n`;
    code += `    metrics: ['mse']\n`;
    code += `  });\n`;
    code += `  console.log('已创建线性回归模型');\n`;
