...
blockly.initLanguage = function () {
  ...
  document.getElementById('runButton').title = "Run Python code";
...
blockly.LaunchCode = function () {
  ...
  // 显示运行中提示
  var loadingDiv = document.getElementById('code-loading');
  if (!loadingDiv) {
    loadingDiv = document.createElement('div');
    loadingDiv.id = 'code-loading';
    loadingDiv.className = 'code-loading';
    loadingDiv.innerHTML = '<div class="loading-spinner"></div><p>Running Code...</p>';
  ...
  .then(data => {
    ...
    // 创建结果弹出窗口或在页面内显示
    var resultDiv = document.getElementById('code-result');
    if (!resultDiv) {
      resultDiv = document.createElement('div');
      resultDiv.id = 'code-result';
      resultDiv.className = 'code-result-popup';
      document.body.appendChild(resultDiv);
    }
    
    var resultContent = '<div class="code-result-header"><h3>Execution Results</h3><span class="close-btn" onclick="document.getElementById(\'code-result\').style.display=\'none\';">×</span></div>';
    
    resultContent += '<div class="code-section"><h4>Code</h4><pre>' + code + '</pre></div>';
    
    if (data.output) {
      resultContent += '<div class="output-section"><h4>Output</h4><pre>' + data.output + '</pre></div>';
    }
    
    if (data.error) {
      resultContent += '<div class="error-section"><h4>Error</h4><pre class="error-message">' + data.error + '</pre></div>';
    }
    
    if (data.result) {
      resultContent += '<div class="result-section"><h4>Result</h4><pre>' + data.result + '</pre></div>';
    }
